...
run_lasso_iter <- function(iter, trainHAE, trainIdxIterIDNonHAE, wt_list, k.folds, crit, traceFile){
    ...
    lambda_seq<- c(initial_lambda[-length(initial_lambda)] , 
                   seq(initial_lambda[length(initial_lambda)] , 0 , length=20)) 
    ...
    curve_outOfSmp <- get_out_of_sample_curve(optim_wt, optim_lambda, foldid, response, data_mtx)
