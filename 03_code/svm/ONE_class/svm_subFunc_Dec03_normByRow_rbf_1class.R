...
par_onCV <- function(i){
    ...
    cv_data_tr <- cv_trTs_norm$tr #[1] 127032    242
    if(ONE){
        cv_data_tr_H <- subset(cv_data_tr, response==1)
        cv_data_tr <- cv_data_tr_H
        rm(cv_data_tr_H)
    }
    cv_data_tr$response <- as.factor(ifelse(cv_data_tr$response==1, 'H', 'NH'))
    cv_data_ts <- cv_trTs_norm$ts #[1] 31356   242
    ...
    if(!ONE){
          ...
          if(kernel=='lin'){
            grid_ms = c(cost=cost, fold=i, ms=ms)
          }else if(kernel=='rbf'){
            grid_ms = c(cost=cost, gamma=gamma, fold=i, ms=ms)    
          }
...
grid_search_cv_svm_par <- function(r){
    ...
    wts <- ifelse(ONE, list(NULL), list( c("NH"=1,"H"=wt) ))
    cat(file = traceFile, append=T, 'iter-', iter , ' wt-', wt, ' cost-', cost, ' gamma-', gamma, ' start!\n')
    start1 <- proc.time()
    #num_pros <- Sys.getenv('NUMBER_OF_PROCESSORS')
    sfInit(parallel=TRUE, cpus=k.folds, type='SOCK')
    sfSource("F:\\Jie\\Shire_project\\03_code\\svm\\svm_subFunc_Dec03_normByRow_rbf_1class.R")
    ...
    if(!ONE){
        ...
        if(kernel=='lin'){
            ms_avg <- aggregate(ms_onCV_df$ms, by=list(cost=ms_onCV_df$cost), mean, na.rm=T) 
            names(ms_avg) <- c('cost', 'ms')
            
        }else if(kernel=='rbf'){
            ms_avg <- aggregate(ms_onCV_df$ms, by=list(cost=ms_onCV_df$cost, gamma=ms_onCV_df$gamma), mean, na.rm=T)
            names(ms_avg) <- c('cost', 'gamma', 'ms')
...
get_optimum_model_iter_withCV <- function(traceFile, path_sepData, num_pros){
    ...
    opt_wt <- opt_line$wt
    opt_wt <- ifelse(is.null(opt_wt), NA, opt_wt)
    max_ms <- opt_line$ms 
    if(kernel=='rbf'){
    ...
    if(ONE){
        train_data_iter_H <- subset(train_data_iter, response==1)
        train_data_iter <- train_data_iter_H
        rm(train_data_iter_H)
    }
    ...
    rm(test_data_iter)
    wts <-  ifelse(ONE, list(NULL), list( c("NH"=1,"H"=opt_wt) ))
    if(kernel=='lin'){
        svmFit <- myTryCatch(
            svm(
