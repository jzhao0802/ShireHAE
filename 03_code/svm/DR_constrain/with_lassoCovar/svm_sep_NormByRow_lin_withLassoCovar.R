...
kernel <- 'lin'
#k.folds=1
wt_list <-  c(180, 330, 400, 500)
if(ONE){
    wt_list <- wt_list[1]
}
#wt_list <- ifelse(ONE, wt_list[1], wt_list)
cost_list <- c(0.00003, 0.00012,0.0008, 0.008, 0.003, 0.011, 0.013, 0.05)
#cost_list <- (0.01)
...
if(kernel=='lin'){
    ...
    outPath <- paste(rootPath, '04_result\\', model, '\\', folder, '\\iter', iter,  '\\cost(', cost_str, ')_wt(', wt_str, ')_ONE(', ONE, ')_rn_DR0(', DR_onPrb0, ')lassoCovar(', with_lassoCovar, ')', sep='')
    
}else if(kernel=='rbf'){
    grid <- expand.grid(cost = cost_list, gamma = gamma_list, weights = wt_list)[,]   
    outPath <- paste(rootPath, '04_result\\', model, '\\', folder, '\\iter', iter,  '\\cost(', cost_str, ')_gm(', gamma_str, ')_wt(', wt_str, ')_ONE(', ONE, ')_rn_DR0(', DR_onPrb0, ')lassoCovar(', with_lassoCovar, ')', sep='')
...
source("F:\\Jie\\Shire_project\\03_code\\svm\\DR_constrain\\with_lassoCovar\\svm_subFunc_svm_withFpConstrain_withLassoCovar.R")

#load the covariates list lasso retained
#load('D:\\Shire_project\\04_result\\Lasso\\fromHui\\opt_lasso_retained_var.RData')
load('F:\\Jie\\Shire_project\\04_result\\Lasso\\fromHui\\opt_lasso_retained_var.RData')
