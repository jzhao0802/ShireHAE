...
grid <- expand.grid(cost = cost_list, weights = wt_list)[1,]
...
wt_str <- paste(grid[2], collapse=',')
cost_str <- paste(grid[1], collapse=',')
...
source("D:\\Shire_project\\03_code\\Jie\\svm\\svm_subFunc_sep.R")
...
for(iter in 1:n.iter){
    ...
    ctrl <- trainControl(method='cv', number=k.folds, classProbs=TRUE, 
                         summaryFunction = get_auprSummary)
    wtsvmlinear <- get_svmWtModel()
    start <- proc.time()
    svmFit_lin <- train(response~., data=train_data_iter, method=wtsvmlinear, 
                        tuneGrid=grid,
                        trControl=ctrl,
                        metric='DR',
                        maximize=F,
    ...
    result_list[[iter]] <- get_optimum_model_iter(haeIdx_iter, nonhaeIdx_iter, foldid_hae_cv, trainIdxIterIDNonHAE, traceFile, grid)
