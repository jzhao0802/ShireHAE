...
grid_search_cv_svm <- function(r){
    ...
    ms_onCV <- lapply(1:k.folds, function(i){
        ...
        cv_resp_ts <- ifelse(cv_data_ts$response=='H', 1, 0)
        ...
        pred <- predict(svmFit, cv_x_ts , sclae=T,decision.values = TRUE)
    ...
    ms_avg <- aggregate(ms_onCV_df, by=list(cost=ms_onCV_df$cost, wt=ms_onCV_df$weight), mean, na.rm=T)
...
grid_search_cv_svm <- function(r){
    ...
    cat(file=traceFile, append=T,'iter-', iter ,  ' wt-', wt, ' cost-', cost, ' Fold-', i, ' ms_avg-', ms_avg[1, 3], '\n')
    
    return(ms_avg)
}


get_optimum_model_iter_withCV <- function(haeIdx_iter, nonhaeIdx_iter, foldid_hae_cv, trainIdxIterIDNonHAE, traceFile, grid, num_pros){
    
    #num_pros <- Sys.getenv('NUMBER_OF_PROCESSORS')
    sfInit(parallel=TRUE, cpus=num_pros, type='SOCK')
    sfSource("D:\\Shire_project\\03_code\\Jie\\svm\\svm_subFunc_Dec03.R")
    cat(file=traceFile, append=TRUE, 'iter:', iter, ' sfExport running!\n')
    sfExport('haeIdx_iter_forCV', 'nonhaeIdx_iter_forCV'
             , 'foldid_hae_cv', 'trainIdxIterIDNonHAE'
             , 'grid', 'iter', 'traceFile', 'crit')
    sfExport('createCurve', 'grid_search_cv_svm')
    #sfClusterEval(library("glmnet"))
    sfClusterEval(library("e1071"))
    sfClusterEval(library("ROCR"))
    sfClusterEval(library("plyr"))
    ...
    ms_allGrid <- sfClusterApplyLB(1:nrow(grid), grid_search_cv_svm)
    sfStop()
    cat(file=traceFile, append=TRUE, 'iter:', iter, ' parallele grid search end!\n')
    ...
    cat(file=traceFile, append=T, 'iter-', iter , ' wt-', opt_wt, ' cost-', opt_cost, ' svm training on subtrain start! \n')
    start1 <- proc.time()
    test_data_iter$region <- as.numeric(as.factor(test_data_iter$region))
    test_data_iter$gender <- as.numeric(as.factor(test_data_iter$gender))
    ...
    cat(file=traceFile, append=T, 'iter-', iter , ' opt_wt-', opt_wt, ' opt_cost-', opt_cost, ' svm training on training end! '
        , 'time used-', (proc.time()-start1)[3]/60, 'min! '
        , 'ending time-', date() ,'\n'
    )
    
    pred <- predict(svmFit, x_ts2 , sclae=T,decision.values = TRUE)
    cat(file=traceFile, append=T,'iter-', iter , ' opt_wt-', opt_wt, ' opt_cost-', opt_cost, ' pred on left out test end! '
        , 'time used-', (proc.time()-start1)[3]/60, 'min! '
        , 'ending time-', date() ,'\n'
    )
    
    predscore <- attr(pred, "decision.values")
    pred_model <- list(model_onFullTr=svmFit, pred_ts = predscore)
    save(file=paste('Model&Pred_optWt(', opt_wt,')optCost(', opt_cost, ').RData', sep=''), pred_model)
    ms <- createCurve(resp = resp_ts, pred=predscore, recall_tar = crit)
    grid_ms = c(opt_cost=opt_cost, opt_wt=opt_wt, msTs=ms)
    cat(file=traceFile, append=T,'iter-', iter ,  ' wt-', opt_wt, ' cost-', opt_cost, ' msTs-', ms, '\n')
    result_list <- list(resp_pred =data.frame(resp_ts=resp_ts, pred_ts=predscore), ms=grid_ms)
    return(result_list)
}
