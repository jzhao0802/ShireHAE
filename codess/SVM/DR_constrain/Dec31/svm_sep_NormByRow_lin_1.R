...
kernel <- 'lin'
#k.folds=1
wt_list <-  c(200, 300, 400)
if(ONE){
    wt_list <- wt_list[1]
}
#wt_list <- ifelse(ONE, wt_list[1], wt_list)
cost_list <- c(0.00005, 0.0001,0.0005, 0.0013, 0.007, 0.01)
...
if(kernel=='lin'){
    ...
    outPath <- paste(rootPath, '04_result\\', model, '\\', folder, '\\iter', iter,  '\\cost(', cost_str, ')_wt(', wt_str, ')_ONE(', ONE, ')_rn_DR', sep='')
    
}else if(kernel=='rbf'){
    grid <- expand.grid(cost = cost_list, gamma = gamma_list, weights = wt_list)[,]   
    outPath <- paste(rootPath, '04_result\\', model, '\\', folder, '\\iter', iter,  '\\cost(', cost_str, ')_gm(', gamma_str, ')_wt(', wt_str, ')_ONE(', ONE, ')_rn_DR', sep='')
