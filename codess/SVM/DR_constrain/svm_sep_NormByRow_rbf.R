...
#gamma_list <-c(seq(0.001, 0.009, 0.003)
 #              , seq(0.01, 0.09, 0.03)
  #             , seq(0.1, 0.9, 0.1))
gamma_list <- c(0.15, 0.25, 0.35, 0.45, 0.55, 0.65, 0.75, 0.85, 0.95) 
...
if(kernel=='lin'){
    ...
    outPath <- paste(rootPath, '04_result\\', model, '\\', folder, '\\iter', iter,  '\\cost(', cost_str, ')_wt(', wt_str, ')_ONE(', ONE, ')_rn_DR0(', DR_onPrb0, ')', sep='')
    
}else if(kernel=='rbf'){
    grid <- expand.grid(cost = cost_list, gamma = gamma_list, weights = wt_list)[,]   
    outPath <- paste(rootPath, '04_result\\', model, '\\', folder, '\\iter', iter,  '\\cost(', cost_str, ')_gm(', gamma_str, ')_wt(', wt_str, ')_ONE(', ONE, ')_rn_DR0(', DR_onPrb0, ')', sep='')
...
#source("D:\\Shire_project\\03_code\\Jie\\svm\\svm_subFunc_svm_withFpConstrain.R")
