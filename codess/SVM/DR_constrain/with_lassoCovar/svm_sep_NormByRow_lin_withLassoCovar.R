#SVM modeling(both linear kernel and rbf kernel) for k-folds CV using subfunctions() 
...
kernel <- 'lin'
#k.folds=1
wt_list <-  c(10, 100, 400)
if(ONE){
    wt_list <- wt_list[1]
}
#wt_list <- ifelse(ONE, wt_list[1], wt_list)
#cost_list <- c(0.00003, 0.00012,0.0008, 0.008, 0.003, 0.011, 0.013)
cost_list <- c(1, 10, 100, 1000)
...
#gamma_list <-c(seq(0.001, 0.009, 0.003)
 #              , seq(0.01, 0.09, 0.03)
  #             , seq(0.1, 0.9, 0.1))
#gamma_list <- c(0.1, 0.3, 0.5) 
gamma_list <- 0.5
DR <- 190
DR_onPrb0 <- F
over_perc <- c(10, 20, 50)*100
...
over_perc_str <- paste(over_perc, collapse=',')
if(kernel=='lin'){
    grid <- expand.grid(cost = cost_list, weights = wt_list, over_perc=over_perc)[,]
    outPath <- paste(rootPath, '04_result\\', model, '\\', folder, '\\iter', iter
                     ,  '\\cost(', cost_str, ')_wt(', wt_str, ')_ONE(', ONE, ')_rn_DR0(', DR_onPrb0
                     , ')lassoCovar(', with_lassoCovar, ')smote(', smote, ')', sep='')
    
}else if(kernel=='rbf'){
    grid <- expand.grid(cost = cost_list, gamma = gamma_list, weights = wt_list, over_perc=over_perc)[,]   
    outPath <- paste(rootPath, '04_result\\', model, '\\', folder, '\\iter', iter
                     ,  '\\cost(', cost_str, ')_gm(', gamma_str, ')_wt(', wt_str, ')_ONE(', ONE, ')_rn_DR0(', DR_onPrb0
                     , ')lassoCovar(', with_lassoCovar, ')smote(', smote, ')', sep='')
    
}
outPath <- ifelse(smote, paste0(outPath, 'perc(', over_perc_str, ')'), outPath)
n.cpu <- nrow(grid)
...
source("D:\\Shire_project\\03_code\\Jie\\svm\\DR_constrain\\with_lassoCovar\\svm_subFunc_svm_withFpConstrain_withLassoCovar.R")
#load the covariates list lasso retained
