...
par_onCV <- function(i){
  ...
  if(smote){
      ...
      pos.n <- sum(cv_data_tr$response=='H')
      neg.n <- sum(cv_data_tr$response=='NH')
      under_perc <- neg.n/(pos.n*(over_perc/100+1))*100
...
grid_search_cv_svm_par <- function(r){
  ...
  sfExport('grid', 'cost', 'wt', 'wts', 'iter', 'traceFile', 'crit', 'k.folds', 'path_sepData'
           , 'ONE', 'kernel', 'clsMethod', 'gamma', 're_model_var', 'with_lassoCovar', 'smote', 'over_perc')
