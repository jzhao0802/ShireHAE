...
library(snowfall)
library(caTools)
source("D:\\Shire_project\\03_code\\Jie\\sep\\subFunc_sep_v2.R")


rootPath <- "D:\\Shire_project\\"
inPath <- paste(rootPath, '02_data\\',sep='')
#outPath <- paste(inPath, 'preModel', sep='')
outPath <- paste(rootPath, '04_result\\lasso\\sep', sep='')
...
#n.cpu <- nrow(grid)
source("D:\\Shire_project\\03_code\\Jie\\sep\\subFunc_sep_v2.R")
or_coef <- get_or_coef_sep(grid, train_data)
...
for(iter in 1:n.iter){
    ...
    result_list[[iter]] <- get_optimum_model_iter_sep(iter, grid, crit, traceFile)
