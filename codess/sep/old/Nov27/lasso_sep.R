...
library(snowfall)
library(caTools)
source("D:\\Shire_project\\03_code\\Jie\\sep\\Nov27\\subFunc_sep.R")
...
outPath <- paste(rootPath, '04_result\\lasso\\iter', iter, '\\wt(0.1)', sep='')
...
for(iter in 1:n.iter){
    ...
    result_list[[iter]] <- get_optimum_model_iter_sep_v1(haeIdx_iter, nonhaeIdx_iter, foldid_hae_cv, trainIdxIterIDNonHAE, traceFile, grid, lambda_seq)
...
write.csv(resp_pred_df_forTs[, -1], 'pred_score_val_iter_', iter, '.csv', row.names = F)
