...
grid_search_v1_sep <- function(haeIdx_iter, nonhaeIdx_iter, foldid_hae_cv, trainIdxIterIDNonHAE, traceFile, grid, lambda_seq){
    ...
    for(j in wt_list){
        ...
        auc_fromCV <- lapply(1:k.folds, function(i){
            ...
            wt_vct <- ifelse(cv_training_data_lasso$response==1, 1, wt)
            ...
            test_pred_avg<- apply(test_pred, 2, function(x){createCurve(cv_test_data_lasso$response , x, crit)})
            test_pred_avg<- c(test_pred_avg , rep(NA , length(lambda_seq) - length(test_pred_avg))) # some small lambda may not be reached
            optN_cvi <- which(test_pred_avg==max(test_pred_avg))
            optLmd_cvi <- lambda_seq[optN_cvi]
            cat("wt-", j, ' fold-', i, 'optN_cvi:', optN_cvi, 'optLmd_cvi', optLmd_cvi, 'maximum ms-', max(test_pred_avg), ' \n')
            cat(file=traceFile, append=TRUE, "wt-", j, ' fold-', i, 'optN_cvi:', optN_cvi, 'optLmd_cvi', optLmd_cvi,optLmd_cvi, 'maximum ms-', max(test_pred_avg), ' \n')
            cat("wt-", j, ' fold-', i, ' end!\n')
    ...
    opt_wt <- wt_list[which(optMs_allWt[, 2]==max(optMs_allWt[, 2]))]
    optMs <-  optMs_allWt[optMs_allWt[, 2]==max(optMs_allWt[, 2]), ]
...
get_optimum_model_iter_sep_v1 <- function(haeIdx_iter, nonhaeIdx_iter, foldid_hae_cv, trainIdxIterIDNonHAE, traceFile, grid, lambda_seq){
    
    cat(file=traceFile, append=TRUE, 'iter:', iter, ' parallele grid search running!\n')
    cat( 'iter:', iter, ' parallele grid search running!\n')
    
    ms_fromWtFd <- grid_search_v1_sep(haeIdx_iter, nonhaeIdx_iter, foldid_hae_cv, trainIdxIterIDNonHAE, traceFile, grid, lambda_seq)
    ...
    total_model<- glmnet(x=data_mtx, y=train_data_iter$response, 
