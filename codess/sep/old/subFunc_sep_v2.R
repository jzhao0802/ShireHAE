...
grid_search_v2_sep <- function(training_data_lasso, traceFile, grid, foldid, lambda_seq, crit){
    temp1 <- lapply(1:nrow(grid), function(r){
        i <- grid[r, 2]
        j <- grid[r, 1]
        cat(file=traceFile, "wt-", j, ' fold-', i, ' start!\n')
        cat("wt-", j, ' fold-', i, ' start!\n')
        cv_training_data_lasso<- training_data_lasso[foldid!=i,]                                            # select 9 folds as training data
        cv_training_matrix<- model.matrix(response~., data=cv_training_data_lasso)[,-1]
        cv_test_data_lasso<- training_data_lasso[foldid==i,]                                                # select 1 fold as test data
        cv_test_matrix<- model.matrix(response~., data=cv_test_data_lasso)[,-1]
        cat(file=traceFile, "wt-", j, ' fold-', i, ' lasso start!\n')
        ...
        cat(file=traceFile, "wt-", j, ' fold-', i, ' end!\n')
        cat("wt-", j, ' fold-', i, ' end!\n')
...
get_optimum_model_iter <- function(num_pros, iter, grid, crit, traceFile){
  ...
  cat(file=traceFile, 'iter:', iter, ' sfExport running!\n')
  ...
  sfClusterEval(library("plyr"))
  sfClusterEval(library("dplyr"))
  cat(file=traceFile, 'iter:', iter, ' parallele grid search running!\n')
...
get_optimum_model_iter_sep <- function(iter, grid, crit, traceFile){
    
    cat(file=traceFile, 'iter:', iter, ' parallele grid search running!\n')
    cat( 'iter:', iter, ' parallele grid search running!\n')
    ms_fromWtFd <- grid_search_v2_sep(train_data_iter,  traceFile, grid, foldid)
    sfStop()
    cv_auc <- ldply(ms_fromWtFd, quickdf)
...
get_or_coef_sep <- function(grid, training_data_lasso){
    ...
    #lambda_seq <- lambda_seq[1:5]
    # Tenfold cross-validation
    ms_fromWtFd <- grid_search_v2_sep(training_data_lasso, traceFile, grid, foldid, lambda_seq, crit)
    cv_auc <- ldply(ms_fromWtFd, quickdf)
    cat(file=traceFile, 'grid search end!\n')
    ...
    optimum_lmd <- lambda_seq[optimum_lambdaIdx]
    # calculate coeff, odds ratio and p-value
    cat(file=traceFile, 'apply optimum model to the whole training data start!\n')
    ...
    optim_model<- glmnet(x=training_matrix, y=training_data_lasso$response, 
                         ...
                         weights = ifelse(training_data_lasso$response==1, 1, optimum_wt),
                         standardize=F)
    cat(file=traceFile, 'apply optimum model to the whole training data end!\n')
