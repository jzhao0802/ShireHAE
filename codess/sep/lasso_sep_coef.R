...
set.seed(20)
foldid_hae <- createFolds(1:nrow(trainHAE), n.iter, returnTrain=F)
...
n.cpu <- nrow(grid)
lmd_length <- 100
outPath <- paste(rootPath, '04_result\\lasso\\sepCoef\\Nov27\\wt(0.1)', sep='')
...
traceFile <- paste(outPath, '\\traceFile_coef.csv', sep='')


source("D:\\Shire_project\\03_code\\Jie\\sep\\subFunc_sep_Nov28.R")
cat(file=traceFile, append=T, 'coefficient with wt-', wt_list, ' running...!\n')
cat('coefficient with wt-', wt_list, ' running...!\n')

or_coef <- get_or_coef_sep_v1(trainIdxHAE, nonHaeIdx_train, foldid_hae, grid, lmd_length, test=F)
cat('coefficient running\n','wt_list-',wt_list, ' k.folds-', k.folds, 'lambda length-', lmd_length, ' running...\n')
cat(file=traceFile, append=T, 'coefficient running\n', 'wt_list-',wt_list, ' k.folds-', k.folds, 'lambda length-', lmd_length, ' running...\n')
save(file = 'coef and optimum parameter.RData', or_coef)
write.csv(or_coef$coef, 'coefficient_OR.csv', row.names = F)
or_coef$optim_pmt
#foldid_hae
#1   2   3   4   5 
#198 197 197 197 197 
#outPath <- paste(inPath, 'preModel', sep='')
source("F:\\Jie\\Shire\\03_code\\subFunc_sep_Nov28.R")
...
write.csv(resp_pred_df_forTs[, -1], paste('pred_score_val_iter_', iter, '.csv', sep=''), row.names = F)
