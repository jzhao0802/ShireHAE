#some functions
#Shire project
#main function for k-folds cross validation and grid search(weighted + lambda) for lasso and ridge regression
...
grid_search_v1_sep <- function(haeIdx_iter, nonhaeIdx_iter, foldid_hae_cv, trainIdxIterIDNonHAE, traceFile, grid, lambda_seq){
    ...
    for(j in wt_list){
        ...
        auc_fromCV <- lapply(1:k.folds, function(i){
            ...
            save(file=paste('dataset_cv_', i, '_v2.RData', sep=''), cvIdx)
...
file <- 'D:\\Shire_project\\04_result\\Lasso\\iter'
for (i in 1:5){
    file1 <- paste(file, i, sep='')
    file2 <- paste(file1, '\\wt(0.1)', sep='')
    file2 <- paste(file1, '\\wt(0.01,0.2,0.6)', sep='')
    file2 <- paste(file1, '\\wt(0.1)', sep='')
    
    read.csv(paste(file2, '\\lasso_result.csv', sep=''), head=T)
}
