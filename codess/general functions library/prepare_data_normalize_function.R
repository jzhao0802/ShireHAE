...
create_dummy <- function(data, cati_var){
...
normalize_trTs <- function(tr, ts, binary_var, conti_var, cati_var){
    ...
    dummy <- create_dummy(trTs_data_iter, cati_var)$data
...
norm_byIter <- function(iter){
   ...
   sfInit(parallel=TRUE, cpus=k.folds, type='SOCK')
    sfSource("F:\\Jie\\Shire_project\\03_code\\prepare_data_normalize_function.R")
...
sfSource("D:\\Shire_project\\03_code\\Jie\\prepare_data_normalize_function.R")
