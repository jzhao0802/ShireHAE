...
grid_search_v2 <- function(r){
    ...
    cat(file=traceFile, 'wt-', j, ' fold-', i, ' lasso running!\n')
...
get_or_coef <- function(grid, training_data_lasso, num_pros, traceFile){
    ...
    lambda_seq<- c(initial_lambda[-length(initial_lambda)] , seq(initial_lambda[length(initial_lambda)] , 0 , length=200)) # get a length=500 descending sequence from initial lambda to 0
    ...
    cat(file=traceFile, 'export start!\n')
    
    sfExport('training_data_lasso', 'k.folds', 'crit', 'foldid',
             'lambda_seq', 'grid', 'traceFile')
    ...
    cat(file=traceFile, 'optimum model selection end!\n')
    ...
    optim_model<- glmnet(x=training_matrix, y=training_data_lasso$response, 
                         ...
                         standardize=F)
    cat(file=traceFile, 'optimum model running end!\n')
